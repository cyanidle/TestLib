...
public:
	...
	inline dMatrix<Type, rows, cols>& transpose() {
		if (rows != cols)
			throw std::exception("Transposition of this matrix is prohibited! (Nonequal sides)");
		dMatrix<Type, rows, cols> result;
		for (int col = 0; col < m_cols; col++) {
			for (int row = 0; row < m_rows; row++) {
	...
	inline dMatrix<Type, src_rows, cols> operator*(dMatrix<Type, src_rows, src_cols>& src) {
		if (m_rows != src.get_cols())
			throw std::exception("Cant multiply matrices of these sizes! (Or you did try Matrix * Vector? -> Should be Vector * Matrix)");
